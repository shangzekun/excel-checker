    ...
    return [
        {
            ...
            "name": "数据成熟度检查",
            "level": "warning",
            "description": "检查焊接工艺点表（RSW / SPR / FDS）的数据成熟度，可结合对应产品 EBOM 清单执行比对。",
            "enabled": True,
            "details": [
                "1. 连接点号规范性检查：连接点号为“xxx123456_7890”，其中，“xxx”为对应工艺的三位字母，123456代表Fixing Part的后六位数字，最后五位为“下划线”+四位数字。标准点号规范如：“RSW349075_0104”。",
                "2. 材料牌号规范性检查：建立了标准材料牌号规范库，对点表中的材料牌号进行一一比对，判断点表中材料牌号是否规范。",
                "3. 连接点号重复性检查：依次遍历点表中的连接点号，判断连接点号是否唯一。",
                "4. 连接点号位置重复性检查：按照空间位置10mm距离进行检查，两连接点的空间位置相距≤10mm报错。",
                "5. 钣金零件：同一零件版本号一致性检查：依次遍历点表中的零件及其版本号，判断同一零件在该点表中的版本号是否唯一。",
                "6. 钣金零件：同一零件材料牌号一致性检查：依次遍历点表中的零件及其材料牌号，判断同一零件在该点表中的材料牌号是否唯一。",
                "7. 钣金零件：同一零件材料厚度一致性检查：依次遍历点表中的零件及其厚度，判断同一零件在该点表中的厚度是否唯一。由于铸件的厚度不唯一，该检查项并没有对铸件的厚度进行检查。",
                "8. Fixing-Part版本号与BOM中版本号一致性检查：将点表中Fixing-Part的版本号与选中的BOM清单中的版本号进行比对，判断两者是否相同。",
                "9. 零件版本号：点表与BOM一致性检查：将点表中每个零件的版本号与选中的BOM清单中的该零件版本号进行比对，判断两者是否相同。",
                "10. 零件材料牌号：点表与BOM一致性检查：将点表中每个零件的材料牌号与选中的BOM清单中的该零件材料牌号进行比对，判断两者是否相同。",
                "11. 零件厚度：点表与BOM一致性检查：将点表中每个零件的厚度与选中的BOM清单中的该零件厚度进行比对，判断两者是否相同。由于铸件的厚度不唯一，该检查项并没有对铸件的厚度进行检查。",
                "12. 工艺点属性的检查：如果该连接点属于单边点焊或四层搭接，判断该连接点是否被标注为工艺点。如果该连接点被标注为工艺点，判断该连接点是否属于单边点焊或四层搭接，如若不是给出提示，需人工确认该点是否为工艺点。",
            ],
        },
        {
